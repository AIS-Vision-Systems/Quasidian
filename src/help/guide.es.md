...
Si la carpeta — o un ancestro — contiene un marcador de proyecto (`CLAUDE.md`, `.claude`, `AGENTS.md`, `.codex`, `.obsidian` o `.git`, aunque sea un archivo), esa carpeta pasa a ser la raíz de un **vault recursivo**: el árbol muestra las subcarpetas y los enlaces, los backlinks y la búsqueda abarcan todo el proyecto. Las carpetas ocultas (que empiezan por punto) no se escanean por defecto; el marcador funciona igualmente. Activa «Mostrar las carpetas ocultas» en la configuración para incluirlas.
